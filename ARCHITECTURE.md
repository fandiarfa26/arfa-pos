...
| **Route group boundary for product forms?**                          | Product add and edit routes are under `(app)/products/` (outside `(main)`) — they don't show BottomNav. All other app pages (dashboard, POS, list, transactions) are under `(main)`. |
